...
int CSolidSBCConfigFile::Init(void)
{
	if (m_bIsInitialized)
		m_cConfigFile.Close();

	CFileException fileException;
	CString strConfigFileNamePath = _T("");
	strConfigFileNamePath.Format(_T("%s\\%s"),g_strImagePath,SSBC_SRV_CFGFILE);

	if ( !m_cConfigFile.Open( strConfigFileNamePath, CFile::modeReadWrite, &fileException ) ){
		if ( !m_cConfigFile.Open( strConfigFileNamePath, CFile::modeCreate | CFile::modeReadWrite, &fileException ) ){
	
			{
				CString strLog;
				strLog.Format( _T("ERROR: Can't open/create config file at %s, error = %d"), strConfigFileNamePath, fileException.m_cause );
